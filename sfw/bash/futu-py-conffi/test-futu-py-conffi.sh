#!/bin/bash 
# file: sfw/bash/futu-py-conffi/test-futu-py-conffi.sh
...
doRunTests(){
	...
	while read -r action ; do (
		...
		done < <(find sfw/bash/futu-py-conffi/tests -type f -name '*.sh')

		doLog "INFO STOP :: testing action: \"$action\""
	);
	done < <(cat $product_version_dir/sfw/bash/futu-py-conffi/tests/run-futu-py-conffi-tests.lst)
...
doLog(){
   ...
   test -t 1 && echo " [$type_of_msg] `date +%Y.%m.%d-%H:%M:%S` [futu-py-conffi][@$host_name] [$$] $msg "
...
#----------------------------------------------------------
# Purpose:
# a simplistic tester for the simplistic bash stub futu-py-conffier
#----------------------------------------------------------
#
#----------------------------------------------------------
...
#----------------------------------------------------------
#
# eof file: sfw/bash/futu-py-conffi/test-futu-py-conffi.sh
